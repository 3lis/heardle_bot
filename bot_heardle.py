...
#####################################################################################################################
    ...
    Alice Apr 2022   ~   Bot Telegram for Heardle Leaderboard
...
# unicodes used in the message generated by heardle
speaker_0           = "\U0001F50A"
speaker_1           = "\U0001F508"
speaker_2           = "\U0001F507"
block_white         = "\U00002B1C"
block_green         = "\U0001F7E9"
block_red           = "\U0001F7E5"
block_black         = "\U00002B1B"
blank               = "\U0000FE0F"

# structure of the head of the message generated by heardle
pattern             = f"^(#Heardle #[0-9]+\n\n[{speaker_1}{speaker_2}]"
pattern            += f"[{block_white}{block_green}{block_red}{block_black}{blank}]+)"  # beware of the weird blank space char!!
...
def start_conv( update, context ):
    ...
    txt     = "Play [Heardle](https://www.heardle.app/) and share your score! "
...
def save_score( update, context ):
    """ -------------------------------------------------------------------------------------------------------------
    Save the score of the day. User must send a message generated by the Heardle website
    ------------------------------------------------------------------------------------------------------------- """
    user        = update.effective_user.username
    if not check_user( user, update ):
        return
    ...
    if r is None:
        txt     = "ERROR: the bot only accepts messages generated by Heardle"
        msg( update, txt )
        return

    # get day and string score
    day     = int( sent_txt.split( '\n' )[ 0 ].split()[ 1 ][ 1: ] )
    score_s = sent_txt.split( '\n' )[ 2 ]
    ...
    # extract score from emoji string
    if score_s.startswith( speaker_2 ):
        score   = FAIL_S
    else:
        score   = 1
        for c in score_s:
            if c in ( block_red, block_black ):
                score   += 1
            elif c == block_green:
                break
...
def show_help( update, context ):
    ...
    ------------------------------------------------------------------------------------------------------------- """
    txt     = "Play from the official [Heardle website](https://www.heardle.app/).\n"
    txt    += "From the website, share your score through Telegram directly to the bot.\n"
    msg( update, txt )
    img( update, "https://www.nationalworld.com/jpim-static/image/2022/03/05/13/heardle.jpg" )
